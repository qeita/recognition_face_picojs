(() => {
  ...
  let result = {x: 0, y: 0, size: 0}
  let countNum = 0
  let intervalFrameNum = 20
  ...
  faceImg.src = './assets/img/fig_face_01.png'
  ...
  function detectFace(){
    ...
    for(let i = 0; i < dets.length; ++i){
      ...
      if(dets[i][3] > threshold.value){
        ...
        result.y = dets[i][0] - dets[i][2]
        result.size = dets[i][2] * 2
      }else{
        if(!isFaceDetect){
          countNum++
        }
    ...
    if(countNum < intervalFrameNum){
      ctx.drawImage(faceImg, result.x, result.y, result.size, result.size)
    }
